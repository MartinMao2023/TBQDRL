...
component_means = jnp.concatenate([
    ...
    jax.random.normal(subkey_cm, shape=(7 * env.action_size)) * 0.25,
])
...
model_bytes = serialization.to_bytes(ppo_training_state.policy_params)
critic_bytes = serialization.to_bytes(ppo_training_state.critic_params)


folder_path = f"./output/GMM"

with open(folder_path + f"/continue_policy.msgpack", "wb") as f:
    f.write(model_bytes)

with open(folder_path + f"/continue_critic.msgpack", "wb") as f:

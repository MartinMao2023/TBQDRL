...
env = envs.create(env_name="humanoid", episode_length=4096, backend="mjx", auto_reset=True)
...
ppo_config = BinaryOptionCriticPPOConfigs(
    ...
    # state_swap_fraction = 0.25,
    state_swap_fraction = 0.0,
)

option_critic_ppo = BinaryOptionCriticPPO(
    env=env,
    policy_network=policy_network,
    critic_network=critic_network,
    selector_network=selector_network,
    configs=ppo_config,
    # persistence_anneal_fn=lambda x: jnp.maximum(0.8, 0.95 - x * 0.001),
    # persistence_anneal_fn=lambda x: jnp.maximum(0.9, 1.0 - x * 0.0005),
...
states = jax.vmap(lambda x: states)(jnp.zeros(2)) # 2 options stacked in axis zero

loop_random_key, subkey = jax.random.split(loop_random_key)
training_state = option_critic_ppo.init(
    subkey,
    policy_params_0=policy_0_params,
    policy_params_1=policy_1_params,
    critic_params_0=critic_params,
    critic_params_1=critic_params,
    critic_means=jnp.array([moving_mean, moving_mean]),
    critic_stds=jnp.array([moving_std, moving_std]),
    moving_mses=jnp.array([moving_mse, moving_mse]),
)
...
def training_loop(carry, _):
    ...
    resampled_states = jax.vmap(jax.vmap(env.resample_task_state))(final_states)
    loop_random_key, subkey = jax.random.split(loop_random_key)
    ps = jax.random.bernoulli(subkey, p=0.25, shape=(2, vec_env,))

    new_states = jax.tree.map(
        lambda a, b: jax.vmap(jax.vmap(jax.lax.select))(ps, a, b),
        resampled_states,
        final_states,
    )
...
for i in range(int(num_iterations // log_period)):
    ...
    wandb.log({
        "critic RMSE": jnp.mean(stacked_aux_data.critic_rmses),
        "approx kl 0": jnp.mean(stacked_aux_data.policy_approx_kls[:, 0]),
        "approx kl 1": jnp.mean(stacked_aux_data.policy_approx_kls[:, 1]),
        "policy 0 return": jnp.mean(stacked_aux_data.option_returns[:, 0]),
        "policy 1 return": jnp.mean(stacked_aux_data.option_returns[:, 1]),

...
class GMMPPOTransition(flax.struct.PyTreeNode):
    """Stores data corresponding to a transition collected by PPO using GMM distribution"""

    obs: Observation
    actions: Action
    zs: jax.Array # task / history embedding
    inv_likelihood: jax.Array # 1/likelihood, normilized for stability
    log_likelihood: jax.Array # scale only, for stability
    rewards: jax.Array
    td_lambda_returns: jax.Array
    gaes: jax.Array
    dones: Done
    ...
    def flatten_dim(self) -> int:
        ...
        """
        flatten_dim = self.observation_dim + self.action_dim + self.z_dim + 8
        return flatten_dim
    ...
    def from_flatten(
        ...
        zs = flattened_transition[:, obs_dim + action_dim: 
                                     obs_dim + action_dim + task_dim]
        inv_likelihood = flattened_transition[:, obs_dim + action_dim + task_dim: 
                                              obs_dim + action_dim + task_dim + 1]
        log_likelihood = flattened_transition[:, obs_dim + action_dim + task_dim + 1: 
                                              obs_dim + action_dim + task_dim + 2]
        rewards = flattened_transition[:, obs_dim + action_dim + task_dim + 2: 
                                       obs_dim + action_dim + task_dim + 3]
        td_lambda_returns = flattened_transition[:, obs_dim + action_dim + task_dim + 3: 
                                                 obs_dim + action_dim + task_dim + 4]
        gaes = flattened_transition[:, obs_dim + action_dim + task_dim + 4: 
    ...
    @classmethod
    def init_dummy(cls, observation_dim: int, action_dim: int, z_dim: int) -> GMMPPOTransition:
        """
        Initialize a dummy transition that then can be passed to constructors to get
        all shapes right.
        ...
        Returns:
            a dummy transition
        """
        dummy_transition = GMMPPOTransition(
            obs=jnp.zeros(shape=(1, observation_dim)),
            actions=jnp.zeros(shape=(1, action_dim)),
            zs=jnp.zeros(shape=(1, z_dim)),
            inv_likelihood=jnp.zeros(shape=(1, 1)),
            log_likelihood=jnp.zeros(shape=(1, 1)),
            rewards=jnp.zeros(shape=(1, 1)),
            td_lambda_returns=jnp.zeros(shape=(1, 1)),
            gaes=jnp.zeros(shape=(1, 1)),
    ...
    def shuffle(self, key: RNGKey) -> GMMPPOTransition:
        flattened_transitions = self.flatten()
        num_transitions = flattened_transitions.shape[0]
        index = jax.random.permutation(key, num_transitions)
        transitions = self.from_flatten(flattened_transitions[index], self)

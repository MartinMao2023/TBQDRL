...
class  LagrangeOptState(PyTreeNode):
    """Contains training state for the lagrange multiplier."""
    dot_product: float
...
class  QDPPOTrainingState(PyTreeNode):
    ...
    lagrange_opt_state: LagrangeOptState
...
class QDPPO:
    def __init__(
        ...
        if policy_network.learnable_std:
            def policy_loss_fn(
                ...
                fitness_gaes = transitions.fitness_gaes
                loss_cond1 = jax.lax.stop_gradient(log_ratio * gaes <= self._clip_log_ratio * jnp.abs(gaes))
                loss_cond2 = jax.lax.stop_gradient(log_ratio * fitness_gaes <= self._clip_log_ratio * jnp.abs(fitness_gaes))
                loss1 = jnp.mean((jnp.where(loss_cond1, -gaes * ratio, 0.0) - ppo_configs.entropy_gain * entropy))
                loss2 = jnp.mean((jnp.where(loss_cond2, -fitness_gaes * ratio, 0.0) - ppo_configs.entropy_gain * entropy))

                clip_fraction = 1 - 0.5 * (jnp.mean(loss_cond1) + jnp.mean(loss_cond2))
                approx_kl = jnp.mean((ratio - 1.0) - log_ratio)

                return jnp.array([loss1, loss2]), (approx_kl, clip_fraction)
            ...
            def policy_loss_fn(
                ...
                loss_cond1 = jax.lax.stop_gradient(log_ratio * gaes <= self._clip_log_ratio * jnp.abs(gaes))
                loss_cond2 = jax.lax.stop_gradient(log_ratio * fitness_gaes <= self._clip_log_ratio * jnp.abs(fitness_gaes))
                loss1 = jnp.mean(jnp.where(loss_cond1, -gaes * ratio, 0.0))
                loss2 = jnp.mean(jnp.where(loss_cond2, -fitness_gaes * ratio, 0.0))
    ...
    def init(
        ...
        training_state = QDPPOTrainingState(
            ...
            lagrange_opt_state=lagrange_opt_state,
    ...
    def state_update(
        ...
        (policy_params, policy_opt_state, new_lagrange_opt_state, 
         final_approx_kl, final_clip_fraction), _ = jax.lax.scan(
            ...
            (training_state.policy_params, training_state.policy_opt_state, 
             training_state.lagrange_opt_state, 0.0, 0.0),
        ...
        new_training_state = QDPPOTrainingState(
            ...
            critic_opt_state=critic_opt_state,
            fitness_critic_opt_state=fitness_critic_opt_state,
            lagrange_opt_state=new_lagrange_opt_state,
        ...
        training_data = QDTrainingMetrics(
            ...
            # lagrange_multiplier=new_lagrange_opt_state.lagrange_multiplier,
            lagrange_multiplier=new_lagrange_opt_state.dot_product,
    ...
    def train_policy(
        self,
        carry: Tuple[Params, optax.OptState, LagrangeOptState, float, float],
        transitions: QDPPOTransition,
        std: jnp.ndarray,
        fitness_weight: float,
        ) -> Tuple[Tuple[Params, optax.OptState, LagrangeOptState, float, float], Any]:
        """
        ...
        def scan_train_policy(carry, transition_data):
            (
                ...
                current_lagrange_opt_state,
                current_approx_kl,
                current_clip_fraction,
                ) = carry

            policy_jacobian, (approx_kl, clip_fraction) = jax.jacrev(
            ...
            new_lagrange_opt_state = self.update_lagrange_params(policy_jacobian, current_lagrange_opt_state)
            
            coef = 1 + fitness_weight * new_lagrange_opt_state.lagrange_multiplier
            policy_gradient = jax.tree.map(
                lambda x: x[0] * coef + fitness_weight * x[1],
                policy_jacobian
            )
    ...
    def update_lagrange_params(
        self, 
        jacobian: Params, 
        lagrange_opt_state: LagrangeOptState,    
    ) -> LagrangeOptState:

        dot_product_tree = jax.tree.map(
            lambda x: jnp.sum(jnp.prod(x, axis=0)),
            jacobian
            )
        squared_l2_norm_tree = jax.tree.map(
            lambda x: jnp.sum(jnp.square(x[0])),
            jacobian
            )
        dot_product = sum(jax.tree.leaves(dot_product_tree))
        squared_l2_norm = sum(jax.tree.leaves(squared_l2_norm_tree))

        new_dot_product=(1 - self.ema_alpha) * dot_product + \
            self.ema_alpha * lagrange_opt_state.dot_product
        new_squared_l2_norm = (1 - self.ema_alpha) * squared_l2_norm + \
            self.ema_alpha * lagrange_opt_state.squared_l2_norm
        new_opt_state = LagrangeOptState(
            dot_product=new_dot_product,
            squared_l2_norm=new_squared_l2_norm,
            lagrange_multiplier=jnp.clip(-new_dot_product, min=0.0) / (new_squared_l2_norm + 1e-8),
        )
    ...
    def evaluate_rollout(
        ...
        _, (v_values, fitnesses, lifespans) = jax.lax.scan(
            scan_evaluation,

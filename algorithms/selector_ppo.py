"""Selector PPO for the relocation ablation.

A softmax "selection" policy is trained with PPO to decide, at each state, which
of ``component_num + 1`` frozen base policies to follow: option 0 is the frozen
Gaussian PPO *teacher* and options 1..component_num are the components of the
frozen *relocated-only* GMM student.  The teacher and the GMM share the same
std, so the candidate action means (teacher mean + each GMM component mean)
are concatenated, one is selected by the softmax, and a single shared-std
Gaussian noise is added to the chosen mean.  The PPO loss only updates the
selector's logits, with the one-hot selection as the "action".

Structure is adapted from ``algorithms/binary_ppo.py`` (categorical PPO loss,
critic loss, GAE, training scans) with two changes:

  * the rollout *gates* between the frozen teacher and the frozen GMM student
    via a softmax over component_num + 1 options instead of stepping a
    binary-action env;
    ...
    def __init__(
        ...
        def rollout_fn(
            ...
            def play_step_fn(carry):
                ...
                # --- selector: mixture weights over (component_num + 1) options ---
                # option 0  -> teacher Gaussian action mean
                # option i  -> i-th GMM component mean of the relocated student
                selection_logits = policy_network.apply(policy_params, obs, z)  # (C+1,)

                # --- candidate action means (all FROZEN) ---
                t_mean, _ = teacher_network.apply(teacher_params, obs, z)       # (d,)
                s_means, _, _ = student_network.apply(student_params, obs, z)  # (C, d)
                means = jnp.concatenate([t_mean[None, :], s_means], axis=0)       # (C+1, d)

                # sample a component by the mixture weights, then add shared-std noise
                ...
                # Per-component Gaussian log-prob of the sampled action. Since the
                # means and the shared std are frozen, this is constant w.r.t. the
                # selector; store it in `actions` so policy_loss_fn only needs the
                # (trainable) selection logits.
                per_comp_log_gauss = -0.5 * jnp.sum(
                    jnp.square(action - means) / (teacher_std**2 + 1e-6),
                    axis=-1,
                )  # (C+1,)

                # mixture log p(a) = logsumexp_i [w_i + log N(a|mean_i,std)] - logsumexp_i [w_i]
                log_likelihood = nn.logsumexp(selection_logits + per_comp_log_gauss) \
                ...
                transition = PPOTransition(
                    obs=obs,
                    actions=per_comp_log_gauss,  # (C+1,) fixed per-component log-probs
                    zs=z,

...
from algorithms.binary_ppo import PPO, PPOConfigs, PPOTrainingState
# from data_struct.transitions import PPOTransition
from networks import GCMLP, GC_BinaryPPO_Policy, ComplexGCMLP, ComplexGCPPO_Policy
# from functools import partial
from flax import serialization
from task_wrappers.cartpole_wrapper import CartpoleWrapper
...
policy_epochs = 4
critic_epochs = 4
policy_learning_rate = 3e-4 # unified
...
env = envs.create(env_name="inverted_pendulum", episode_length=4096, backend="mjx", auto_reset=True)
env = CartpoleWrapper(env)

structure = "simple"
critic_hidden_layers: Tuple[int, ...] = (64, 64)
actor_hidden_layers: Tuple[int, ...] = (64, 64)
policy_network = GC_BinaryPPO_Policy(
...
for i in range(int(num_iterations / log_period)):
    ...
    wandb.log({
        ...
        "iteration mean lifespan": jnp.mean(iteration_mean_lifespan),
        })
